...
class BaseTestManager {
    constructor(testProvider, product, workspaceFolder, rootDirectory, serviceContainer) {
        ...
        this._status = types_2.TestStatus.Unknown;
        this._status = types_2.TestStatus.Unknown;
        ...
        this._outputChannel = this.serviceContainer.get(types_1.IOutputChannel, constants_2.TEST_OUTPUT_CHANNEL);
        this.testCollectionStorage = this.serviceContainer.get(types_2.ITestCollectionStorageService);
        this._testResultsService = this.serviceContainer.get(types_2.ITestResultsService);
    }
    get outputChannel() {
    ...
    get installer() {
        if (!this._installer) {
            this._installer = this.serviceContainer.get(types_1.IInstaller);
        }
    ...
    get workingDirectory() {
        const settings = configSettings_1.PythonSettings.getInstance(vscode_1.Uri.file(this.rootDirectory));
    ...
    discoverTests(cmdSource, ignoreCache = false, quietMode = false, userInitiated = false) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!ignoreCache && this.tests && this.tests.testFunctions.length > 0) {
                this._status = types_2.TestStatus.Idle;
                return Promise.resolve(this.tests);
            }
            this._status = types_2.TestStatus.Discovering;
            ...
            const discoveryService = this.serviceContainer.get(types_2.ITestDiscoveryService, this.testProvider);
            return discoveryService.discoverTests(discoveryOptions)
                ...
                if (haveErrorsInDiscovering && !quietMode) {
                    const testsHelper = this.serviceContainer.get(types_2.ITestsHelper);
                    testsHelper.displayTestErrorMessage('There were some errors in discovering unit tests');
                }
                const wkspace = vscode_1.workspace.getWorkspaceFolder(vscode_1.Uri.file(this.rootDirectory)).uri;
                ...
                if (this.testDiscoveryCancellationToken && this.testDiscoveryCancellationToken.isCancellationRequested) {
                    ...
                    index_1.sendTelemetryEvent(constants_1.UNITTEST_DISCOVER, undefined, telementryProperties);
                    this._status = types_2.TestStatus.Error;
    ...
    runTest(cmdSource, testsToRun, runFailedTests, debug) {
        ...
        this._status = types_2.TestStatus.Running;
        if (this.testRunnerCancellationTokenSource) {
            this.testRunnerCancellationTokenSource.cancel();
        }
