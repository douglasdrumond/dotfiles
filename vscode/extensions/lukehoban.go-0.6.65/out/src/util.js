...
function getCurrentGoPath() {
    let configGopath = vscode.workspace.getConfiguration('go')['gopath'];
    let inferredGopath;
    if (vscode.workspace.getConfiguration('go')['inferGopath'] === true) {
        inferredGopath = goPath_1.getInferredGopath(vscode.workspace.rootPath);
    }
    return inferredGopath ? inferredGopath : (configGopath ? goPath_1.resolvePath(configGopath, vscode.workspace.rootPath) : process.env['GOPATH']);
}
