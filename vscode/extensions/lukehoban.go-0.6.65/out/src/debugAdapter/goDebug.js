    ...
    constructor(remotePath, port, host, program, launchArgs) {
        ...
        this.connection = new Promise((resolve, reject) => {
            ...
            if (!fileEnv['GOPATH'] && !launchArgs.env['GOPATH'] && (mode === 'debug' || mode === 'test')) {
                ...
                let programNotUnderGopath = !env['GOPATH'] || !goPath_1.getCurrentGoWorkspaceFromGOPATH(env['GOPATH'], isProgramDirectory ? program : path.dirname(program));
                if (programNotUnderGopath) {
                    env['GOPATH'] = goPath_1.getInferredGopath(isProgramDirectory ? program : path.dirname(program)) || env['GOPATH'];
                }
            }
            ...
            let dlv = goPath_1.getBinPathWithPreferredGopath('dlv', goPath_1.resolvePath(env['GOPATH']), process.env['GOPATH']);
