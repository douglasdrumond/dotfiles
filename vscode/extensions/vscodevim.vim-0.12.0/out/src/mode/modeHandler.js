            ...
            for (const command of otherTransformations) {
                switch (command.type) {
                    ...
                    case 'showCommandHistory':
                        let cmd = yield commandLine_1.CommandLine.ShowHistory(vimState.commandInitialText, this.vimState);
                        if (cmd && cmd.length !== 0) {
                            yield commandLine_1.CommandLine.PromptAndRun(cmd, this.vimState);
                            this.updateView(this.vimState);
                        }
                        break;
                    case 'dot':
